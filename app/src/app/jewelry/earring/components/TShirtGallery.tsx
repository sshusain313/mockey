...
export function TShirtGallery({ activeCategory, setActiveCategory, products = [], uploadedDesign }: TShirtGalleryProps) {
  ...
  useEffect(() => {
    console.log(`TShirtGallery: Received ${products.length} earring products, uploadedDesign: ${uploadedDesign ? 'present' : 'not present'}`);
    if (products.length > 0) {
      console.log('First earring product sample:', products[0]);
    } else {
      console.log('No earring products received from server');
    }
