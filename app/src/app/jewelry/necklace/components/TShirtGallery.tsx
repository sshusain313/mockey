...
export function TShirtGallery({ activeCategory, setActiveCategory, products = [], uploadedDesign }: TShirtGalleryProps) {
  ...
  // Debug: Log products received and uploadedDesign status using useEffect to prevent infinite loops
  useEffect(() => {
    console.log(`TShirtGallery: Received ${products.length} necklace products, uploadedDesign: ${uploadedDesign ? 'present' : 'not present'}`);
    if (products.length > 0) {
      console.log('First necklace product sample:', products[0]);
    } else {
      console.log('No necklace products received from server');
    }
  }, [products, uploadedDesign]); // Only re-run when products or uploadedDesign changes
