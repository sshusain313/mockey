...
// Get mock tube products for fallback
function getMockTubeProducts() {
  return [
    {
      _id: 'tube-1',
      name: 'Cosmetic Tube',
      description: 'Premium cosmetic tube for creams and lotions with customizable design.',
      category: 'packaging',
      subcategory: 'cosmetic tube',
      price: 18.99,
    ...
    },
    {
      _id: 'tube-2',
      name: 'Packaging Tube',
      description: 'Versatile packaging tube with customizable branding.',
      category: 'packaging',
      subcategory: 'packaging tube',
      price: 22.99,
...
/**
 * Fetch only tube products
 * This function specifically filters for products with subcategory related to tubes
 */
export const getTubeProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    // Create a query to fetch only tube products
    const query = {
      ...
      // Filter for tube subcategory (case-insensitive)
      $or: [
        { subcategory: { $regex: 'tube', $options: 'i' } },
        { subcategory: { $regex: 'cosmetic tube', $options: 'i' } },
        { subcategory: { $regex: 'packaging tube', $options: 'i' } },
        { subcategory: { $regex: 'cream tube', $options: 'i' } },
        { subcategory: { $regex: 'lotion tube', $options: 'i' } },
        { subcategory: { $regex: 'toothpaste tube', $options: 'i' } }
      ]
    };
    
    // Fetch tube products from MongoDB with the specific query
    ...
    if (dbProducts.length > 0) {
      ...
      console.log('Found tube subcategories:', subcategories);
    }
    
    // Convert MongoDB documents to plain objects
    products = dbProducts.map(product => formatProductData(product));
    
    console.log(`Server: Fetched ${products.length} tube products with complete data`);
    
    // If no tube products found in the database, use mock tube data
    if (products.length === 0) {
      console.log('No tube products found in database, using mock tube data');
      products = getMockTubeProducts();
    }
  } catch (error) {
    console.error('Error fetching tube products:', error);
    // Use mock tube data as fallback
    products = getMockTubeProducts();
  }
