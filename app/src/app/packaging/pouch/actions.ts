...
// Get mock pouch products for fallback
function getMockPouchProducts() {
  return [
    {
      _id: 'pouch-1',
      name: 'Stand-up Pouch',
      description: 'Durable stand-up pouch with customizable design area.',
      category: 'packaging',
    ...
    },
    {
      _id: 'pouch-2',
      name: 'Ziplock Pouch',
      description: 'Resealable ziplock pouch with customizable branding.',
      category: 'packaging',
      subcategory: 'ziplock pouch',
      price: 22.99,
...
/**
 * Fetch only pouch products
 * This function specifically filters for products with subcategory related to pouches
 */
export const getPouchProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    // Create a query to fetch only pouch products
    const query = {
      ...
      // Filter for pouch subcategory (case-insensitive)
      $or: [
        { subcategory: { $regex: 'pouch', $options: 'i' } },
        { subcategory: { $regex: 'stand-up pouch', $options: 'i' } },
        { subcategory: { $regex: 'ziplock pouch', $options: 'i' } },
        { subcategory: { $regex: 'food pouch', $options: 'i' } },
        { subcategory: { $regex: 'packaging pouch', $options: 'i' } },
        { subcategory: { $regex: 'flexible packaging', $options: 'i' } }
      ]
    };
    
    // Fetch pouch products from MongoDB with the specific query
    ...
    if (dbProducts.length > 0) {
      ...
      console.log('Found pouch subcategories:', subcategories);
    }
    
    // Convert MongoDB documents to plain objects
    products = dbProducts.map(product => formatProductData(product));
    
    console.log(`Server: Fetched ${products.length} pouch products with complete data`);
    
    // If no pouch products found in the database, use mock pouch data
    if (products.length === 0) {
      console.log('No pouch products found in database, using mock pouch data');
      products = getMockPouchProducts();
    }
  } catch (error) {
    console.error('Error fetching pouch products:', error);
    // Use mock pouch data as fallback
    products = getMockPouchProducts();
  }
