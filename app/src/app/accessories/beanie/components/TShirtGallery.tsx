...
export function TShirtGallery({ 
  ...
  const categories = products.length > 0 ? extractCategories() : [
    "Blank", "Beanie", "Winter", "White", "Black", "Navy", 
    "Gray", "Wool", "Acrylic", "Pom-pom", "Cuffed"
  ];
  ...
  const mockups: MockupItem[] = filteredMockups.length > 0 ? filteredMockups : [
    {
      id: "1",
      title: "Classic Black Beanie",
      image: "/products/beanie-black.jpg",
      categories: ["Black", "Beanie", "Winter"],
      isPro: false,
      placeholder: {
        x: 150,
        y: 120,
    ...
    },
    {
      ...
      image: "/products/beanie-gray.jpg",
      categories: ["Gray", "Beanie", "Wool"],
      isPro: false,
      placeholder: {
        x: 150,
        y: 120,
    ...
    {
      id: "3",
      title: "Navy Cuffed Beanie",
      image: "/products/beanie-navy.jpg",
      categories: ["Navy", "Beanie", "Cuffed"],
      isPro: false,
      placeholder: {
        x: 150,
        y: 120,
  ...
  return (
    <div className="flex-1 overflow-y-auto p-2 sm:p-4 w-full">
      ...
      {/* Category Navigation - Responsive Accordion */}
      <div className="border-b border-gray-200 mb-4">
        <button
          onClick={() => toggleSection('accessories')}
          className="p-3 sm:p-4 w-full text-left flex justify-between items-center hover:bg-gray-50 transition-colors"
        >
        ...
        {expanded.accessories && (
          <div className="py-1">
            {["HOME & LIVING", "PRINT", "PACKAGING", "TECH", "JEWELRY"].map((item) => (
              <button
                key={item}
                className="px-3 sm:px-4 py-2 w-full text-left text-xs sm:text-sm text-gray-700 hover:bg-gray-50 block transition-colors"
              >
                {item}
              </button>
            ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
        {mockups.map((mockup) => (
          <div 
            key={mockup.id} 
            className="rounded-lg overflow-hidden border border-gray-200 group cursor-pointer hover:shadow-md transition-all duration-200 bg-white"
            onClick={() => handleMockupClick(mockup)}
          >
            <div className="relative aspect-square bg-gray-100">
              {mockup.isPro && (
                <div className="absolute top-2 right-2 bg-amber-500 text-white text-xs font-bold px-1.5 py-0.5 rounded z-10">
                  PRO
                </div>
              )}
              <Image 
              ...
              {/* Design Overlay - Responsive Positioning */}
              {uploadedDesign ? (
                <div 
                ...
                /* Placeholder Area - Responsive Design */
                <div 
                  className="absolute inset-0 pointer-events-none"
                  style={{
                    position: 'absolute',
              ...
              {/* Edit Icon - Responsive Sizing */}
              <div className="absolute bottom-2 right-2 bg-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm">
            ...
            <div className="p-2 sm:p-3">
              ...
              <p className="text-xs text-gray-500 mt-1 hidden sm:block">Click to customize</p>
              <p className="text-[10px] text-gray-500 mt-1 sm:hidden">Tap to edit</p>
            </div>
          </div>
        ))}
        ...
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={`placeholder-${index}`} className="rounded-lg overflow-hidden border border-gray-200 bg-white hover:shadow-sm transition-shadow duration-200">
            <div className="aspect-square bg-gray-100"></div>
            <div className="p-2 sm:p-3">
              <h3 className="text-xs sm:text-sm font-medium text-gray-900 truncate">Beanie Mockup Example</h3>
              <p className="text-xs text-gray-500 mt-1 hidden sm:block">Coming soon</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* How-to Section - Responsive Layout */}
      <section className="bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow-sm max-w-9xl mx-4 sm:mx-10 my-8 sm:my-10 flex flex-col md:flex-row gap-4 sm:gap-6 items-center">
        <div className="md:w-2/3">
          ...
          <p className="text-gray-700 mb-2 text-sm sm:text-base">To design a 3D mockup with Mockey.ai:</p>
