 ...
 */
export const getApronProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'apron', $options: 'i' } },
        { subcategory: { $regex: 'aprons', $options: 'i' } },
    ...
    console.log(`Server: Fetched ${products.length} apron products with complete data`);
