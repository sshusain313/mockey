...
export function TShirtGallery({ activeCategory, setActiveCategory, products = [], uploadedDesign }: TShirtGalleryProps) {
  ...
  useEffect(() => {
    console.log(`TShirtGallery: Received ${products.length} sticker products, uploadedDesign: ${uploadedDesign ? 'present' : 'not present'}`);
    if (products.length > 0) {
      console.log('First sticker product sample:', products[0]);
    } else {
      console.log('No sticker products received from server');
    }
