...
// Get mock banner products for fallback
function getMockBannerProducts() {
  return [
    {
      _id: 'banner-1',
      name: 'Roll-up Banner',
      description: 'Professional roll-up banner for events and exhibitions.',
      category: 'print',
    ...
    },
    {
      _id: 'banner-2',
      name: 'Vinyl Banner',
      description: 'Durable vinyl banner for outdoor advertising.',
      category: 'print',
      subcategory: 'vinyl banner',
      price: 22.99,
 ...
 */
export const getBannerProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'banner', $options: 'i' } },
        { subcategory: { $regex: 'roll-up banner', $options: 'i' } },
        { subcategory: { $regex: 'vinyl banner', $options: 'i' } },
        { subcategory: { $regex: 'mesh banner', $options: 'i' } },
        { subcategory: { $regex: 'fabric banner', $options: 'i' } },
        { subcategory: { $regex: 'display banner', $options: 'i' } }
      ]
    };
    ...
    console.log(`Server: Fetched ${products.length} banner products with complete data`);
    
    // If no banner products found in the database, use mock banner data
    if (products.length === 0) {
      console.log('No banner products found in database, using mock banner data');
      products = getMockBannerProducts();
    }
  } catch (error) {
    console.error('Error fetching banner products:', error);
    // Use mock banner data as fallback
    products = getMockBannerProducts();
  }
