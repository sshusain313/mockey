...
// Get mock flyer products for fallback
function getMockFlyerProducts() {
  return [
    {
      _id: 'flyer-1',
      name: 'A5 Flyer',
      description: 'Standard A5 flyer for promotional events and marketing.',
      category: 'print',
      subcategory: 'a5 flyer',
      price: 18.99,
    ...
    },
    {
      _id: 'flyer-2',
      name: 'A4 Flyer',
      description: 'Professional A4 flyer for business advertising.',
      category: 'print',
      subcategory: 'a4 flyer',
      price: 22.99,
 ...
 */
export const getFlyerProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'flyer', $options: 'i' } },
        { subcategory: { $regex: 'a5 flyer', $options: 'i' } },
        { subcategory: { $regex: 'a4 flyer', $options: 'i' } },
        { subcategory: { $regex: 'promotional flyer', $options: 'i' } },
        { subcategory: { $regex: 'event flyer', $options: 'i' } },
        { subcategory: { $regex: 'business flyer', $options: 'i' } }
      ]
    };
    ...
    console.log(`Server: Fetched ${products.length} flyer products with complete data`);
    
    // If no flyer products found in the database, use mock flyer data
    if (products.length === 0) {
      console.log('No flyer products found in database, using mock flyer data');
      products = getMockFlyerProducts();
    }
  } catch (error) {
    console.error('Error fetching flyer products:', error);
    // Use mock flyer data as fallback
    products = getMockFlyerProducts();
  }
