...
// Get mock brochure products for fallback
function getMockBrochureProducts() {
  return [
    {
      _id: 'brochure-1',
      name: 'Tri-fold Brochure',
      description: 'Professional tri-fold brochure for business marketing.',
      category: 'print',
      subcategory: 'tri-fold brochure',
      price: 18.99,
    ...
    },
    {
      ...
      description: 'Classic bi-fold brochure for product catalogs.',
      category: 'print',
      subcategory: 'bi-fold brochure',
      price: 22.99,
 ...
 */
export const getBrochureProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'brochure', $options: 'i' } },
        { subcategory: { $regex: 'tri-fold brochure', $options: 'i' } },
        { subcategory: { $regex: 'bi-fold brochure', $options: 'i' } },
        { subcategory: { $regex: 'leaflet', $options: 'i' } },
        { subcategory: { $regex: 'pamphlet', $options: 'i' } },
        { subcategory: { $regex: 'catalog', $options: 'i' } }
      ]
    };
    ...
    console.log(`Server: Fetched ${products.length} brochure products with complete data`);
    
    // If no brochure products found in the database, use mock brochure data
    if (products.length === 0) {
      console.log('No brochure products found in database, using mock brochure data');
      products = getMockBrochureProducts();
    }
  } catch (error) {
    console.error('Error fetching brochure products:', error);
    // Use mock brochure data as fallback
    products = getMockBrochureProducts();
  }
