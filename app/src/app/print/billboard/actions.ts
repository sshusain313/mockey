...
// Get mock billboard products for fallback
function getMockBillboardProducts() {
  return [
    {
      _id: 'billboard-1',
      name: 'Standard Billboard',
      description: 'Large format billboard for impactful outdoor advertising.',
      category: 'print',
    ...
    },
    {
      ...
      description: 'High-resolution digital billboard for dynamic advertising.',
      category: 'print',
      subcategory: 'digital billboard',
      price: 22.99,
 ...
 */
export const getBillboardProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'billboard', $options: 'i' } },
        { subcategory: { $regex: 'digital billboard', $options: 'i' } },
        { subcategory: { $regex: 'outdoor billboard', $options: 'i' } },
        { subcategory: { $regex: 'advertising board', $options: 'i' } },
        { subcategory: { $regex: 'hoarding', $options: 'i' } },
        { subcategory: { $regex: 'large format', $options: 'i' } }
      ]
    };
    ...
    console.log(`Server: Fetched ${products.length} billboard products with complete data`);
    
    // If no billboard products found in the database, use mock billboard data
    if (products.length === 0) {
      console.log('No billboard products found in database, using mock billboard data');
      products = getMockBillboardProducts();
    }
  } catch (error) {
    console.error('Error fetching billboard products:', error);
    // Use mock billboard data as fallback
    products = getMockBillboardProducts();
  }
