...
// Get mock bottle products for fallback
function getMockBottleProducts() {
  return [
    {
      _id: 'bottle-1',
      name: 'Glass Water Bottle',
      description: 'A premium glass water bottle for your custom beverage designs.',
      category: 'packaging',
    ...
    },
    {
      _id: 'bottle-2',
      name: 'Wine Bottle',
      description: 'An elegant wine bottle for your custom wine label designs.',
      category: 'packaging',
      subcategory: 'wine bottle',
      price: 22.99,
 ...
 */
export const getBottleProducts = cache(async () => {
  let products = [];
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        { subcategory: { $regex: 'bottle', $options: 'i' } },
        { subcategory: { $regex: 'water bottle', $options: 'i' } },
        { subcategory: { $regex: 'wine bottle', $options: 'i' } },
        { subcategory: { $regex: 'soda bottle', $options: 'i' } },
        { subcategory: { $regex: 'glass bottle', $options: 'i' } },
        { subcategory: { $regex: 'plastic bottle', $options: 'i' } }
      ]
    };
    ...
    console.log(`Server: Fetched ${products.length} bottle products with complete data`);
    
    // If no bottle products found in the database, use mock bottle data
    if (products.length === 0) {
      console.log('No bottle products found in database, using mock bottle data');
      products = getMockBottleProducts();
    }
  } catch (error) {
    console.error('Error fetching bottle products:', error);
    // Use mock bottle data as fallback
    products = getMockBottleProducts();
  }
